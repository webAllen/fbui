;(function(window, $) {
	...
	//正则：验证是否为空
	fbUi.expEmpty = function(test){  vtf
		var test = test;
	    if(test.length == 0){ 
	        // alert("手机号码有误，请重填");  
	        return false; 
			...
			var html = '<div class="fb-position-absolute fb-waterfall-item" style="width:'+b.width+'px;height:'+b.height+'px;top:'+heightArray[min_index]+'px;left:'+(parseFloat(b.width)+parseFloat(fb.margin))*min_index+'px;">\
				<a href="'+b.href+'">\
					...
					<div class="fb-text fb-position-absolute fb-table fb-transition">\
						<span class="fb-font22 fb-tableCell">'+b.name+'</span>\
					</div>\
				</a>\
			</div>';
	...
	fbUi.prototype.magnifier = function(options){
		...
		$el.on("mousemove",function(e){
			...
			var left = e.pageX - osmall.offset().left - 120 ; 
			var top =  e.pageY - osmall.offset().top - 120 ;
			left = left < 0 ? 0 : left;
			left = left > 420 ? 420 : left;
			top = top > 200? 200 : top;
			top = top < 0 ? 0 : top;
			imgHover.css({"top":top,"left":left})
			var bigleft = left * (3/2) ;
			var bigtop = top * (3/2) ;
